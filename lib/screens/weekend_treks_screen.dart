...
class _WeekendTreksScreenState extends State<WeekendTreksScreen>
  ...
  // Track selected chip locally so all chips always stay visible
  late String _selectedChipDate;

  // Full weekend date chips as yyyy-MM-dd strings (fixed from dashboard)
  late final List<String> _chipDates;
  ...
  void initState() {
    ...
    // Build chip dates from widget.weekendDates (always the full Thu/Fri/Sat set)
    _chipDates = widget.weekendDates
        .map((d) => '${d.year}-${d.month.toString().padLeft(2,'0')}-${d.day.toString().padLeft(2,'0')}')
        .toList();

    // Default selected chip = first match with widget.date (converted), else first chip
    final convertedDate = TrekController.convertDateYYYYMMDD(widget.date);
    _selectedChipDate = _chipDates.contains(convertedDate)
        ? convertedDate
        : (_chipDates.isNotEmpty ? _chipDates.first : convertedDate);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      _trekC.fetchWeekendTreks(
        cityId: widget.city,
        trekId: widget.trek,
        date: _selectedChipDate,
        refresh: true,
      );
    });
  }
  ...
  Widget _buildDateChips(SearchContextModel? ctx) {
    if (_chipDates.isEmpty) return const SizedBox.shrink();
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemCount: _chipDates.length,
            itemBuilder: (_, i) {
              final date  = _chipDates[i];
              final isSel = date == _selectedChipDate;
              return GestureDetector(
                onTap: () async {
                  setState(() => _selectedChipDate = date);
