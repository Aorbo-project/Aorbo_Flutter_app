...
class TrekController extends GetxController {
  ...
  static String convertDateYYYYMMDD(String date) {
    ...
    try {
      // Already yyyy-MM-dd (e.g. from backend weekend_dates chips)
  ...
  Future<void> fetchWeekendTreks({
    ...
    try {
      ...
      if (body != null) {
        final responseData = FetchTreksResponseModel.fromJson(body);
        // success:false with empty/null data = no results, not a real error
        final isNoResults = responseData.success == false &&
            (responseData.data == null || responseData.data!.isEmpty);
        if (responseData.success == true || isNoResults) {
          observer.value.data.value.maybeWhen(success: (data) {
            final oldList =
            (data as FetchTreksResponseModel?)?.data?.toList();
            oldList?.addAll(responseData.data ?? List.empty());
  ...
  Future<String?> fetchCancellationDetails(String bookingId) async {
    try {
      ...
      CustomSnackBar.show(Get.context!, message: e.toString());
      cancellationDetailsResponseObserver.value = ApiResult.error('Failed to get cancellation details: ${e.toString()}');
      return e.toString();
    }
  }
  ...
  Future<String?> reqCancellation(String bookingId) async {
    try {
      ...
      final response = await repository.postApiCall(url: NetworkUrl.refund,body: {"booking_id":int.tryParse(bookingId) ?? bookingId,"reason":cancellationReasonController.value.text.toString()});
      ...
      CustomSnackBar.show(Get.context!, message: e.toString());
      requestCancellationResponseObserver.value = ApiResult.error(e.toString());
      return e.toString();
    }
  }
