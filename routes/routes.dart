...
import '../models/know_more_data.dart';
...
final routes = [
  ...
  GetPage(
    ...
    page: () {
      ...
      return WeekendTreksScreen(
        ...
        trek: args['trek'] ?? '',
        date: args['date'] ?? '',
        weekendDates: _safeList<DateTime>(args['weekendDates']),
      );
    },
  ),
  ...
  GetPage(
    ...
    page: () {
      final discountCard = _safeArg<DiscountCardModel>('discountCard');
      if (discountCard == null) {
        return _invalidArgsScreen('Missing discount card details.');
      }
      return DiscountCardDetailsScreen(discountCard: discountCard);
    },
  ),
  GetPage(
    name: '/know-more-details',
    page: () {
      final knowMoreData = _safeArg<KnowMoreData>('knowMoreData');
      if (knowMoreData == null) {
        return _invalidArgsScreen('Missing know-more details.');
      }
      return KnowMoreDetailsScreen(knowMoreData: knowMoreData);
    },
  ),
  ...
  GetPage(name: '/refers', page: () => const ReferAndEarnScreen()),
  GetPage(name: '/claim', page: () => const ClaimsScreen()),
  GetPage(
...
Map<String, dynamic> _safeArgsMap() {
  final args = Get.arguments;
  return args is Map<String, dynamic> ? args : <String, dynamic>{};
}

T? _safeArg<T>(String key) {
  final args = _safeArgsMap();
  final value = args[key];
  return value is T ? value : null;
}

List<T> _safeList<T>(dynamic value) {
  if (value is List<T>) return value;
  return <T>[];
}

Widget _invalidArgsScreen(String message) {
  return Scaffold(
    appBar: AppBar(
